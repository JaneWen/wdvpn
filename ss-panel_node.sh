...
install_ss_panel(){
	...
	install_soft_for_each
	wget -c https://github.com/JaneWen/wdvpn/blob/master/lnmp1.3.zip && unzip lnmp1.3.zip && cd lnmp1.3 && chmod +x install.sh && ./install.sh lnmp
	chattr -i /home/wwwroot/default/.user.ini
	rm -rf /home/wwwroot/default
	git clone https://github.com/JaneWen/ss-panel.git "/home/wwwroot/default"
	...
	wget -N -P  /usr/local/nginx/conf/ https://github.com/JaneWen/wdvpn/blob/master/nginx.conf 
	lnmp nginx restart
}
...
install_ss_py_mu(){
	...
	install_soft_for_each(){
		...
		if [[ ${release} = "centos" ]]; then
			...
			wget https://github.com/JaneWen/wdvpn/blob/master/libsodium-1.0.11.tar.gz
...
one_click_all(){
	...
	install_soft_for_each
	echo "Let's setup your ssnode/root"
	git clone https://github.com/JaneWen/shadowsocks-py-mu.git "/root/shadowsocks-py-mu"
	#modify Config.py
	echo -e "modify Config.py...\n"
